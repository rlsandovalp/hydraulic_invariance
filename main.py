...
# Create fields in the table of rain
[arcpy.AddField_management(complete_dissolve,field_name,"DOUBLE") for field_name in ['area', 'a1', 'n1', 'alpha', 'epsilon', 'kappa', 'w_50', 'a_50', 'pe_50', 'h_50', 'w_100', 'a_100', 'pe_100', 'h_100', 'CN', 'fo', 'fc', 'hor_exp', 'runoff_c', 'ks', 'ws', 'suc', 'S']]
# Compute the fields created in the previous line
...
field_map = 'POINT_Y "POINT_Y" true true false 19 Double 0 0 ,First,#,'+str(nodes)+',POINT_Y,-1,-1;POINT_X "POINT_X" true true false 19 Double 0 0 ,First,#,'+str(nodes)+',POINT_X,-1,-1;Z "Z" true true false 19 Double 0 0 ,First,#,'+str(nodes)+',Z,-1,-1;h_50 "h_50" true true false 19 Double 0 0 ,First,#,'+complete_dissolve+',h_50,-1,-1;h_100 "h_100" true true false 19 Double 0 0 ,First,#,'+complete_dissolve+',h_100,-1,-1'
...
arcpy.Dissolve_management(fogne2, fogne3, dissolve_field="POINT_Y;POINT_X", statistics_fields="Z MEAN;h_50 MEAN;h_100 MEAN", multi_part="MULTI_PART", unsplit_lines="DISSOLVE_LINES")
[arcpy.AddField_management(fogne3,field_name,"DOUBLE") for field_name in ['Node', 'Acum50', 'Acum100']]
...
for i in range (len(df_nodes)):
   df_nodes.iloc[i,6] = round(df_nodes.iloc[i,6],1)
...
for i in range(len(df_nodes)):
   acum50 = df_nodes.iloc[i,4]
   acum100 = df_nodes.iloc[i,5]
   inputs_df = df_pipes[df_pipes['end'] == df_nodes.iloc[i,6]]['start']
   algo50 = 0
   algo100 = 0
   flow_input50 = 0
   flow_input100 = 0
   if len(inputs_df) == 0:
      df_nodes.iloc[i,7] = acum50
      df_nodes.iloc[i,8] = acum100
   else:
      ...
      df_nodes.iloc[i,7] = acum50 + algo50
      df_nodes.iloc[i,8] = acum100 + algo100
...
arcpy.SpatialJoin_analysis(target_features=segment2, join_features="Fogne4", out_feature_class=split2, join_operation="JOIN_ONE_TO_ONE", join_type="KEEP_ALL", field_mapping='Q "Q" true true false 19 Double 0 0 ,First,#,segment2,Q,-1,-1;acum50 "acum50" true true false 19 Double 0 0 ,Min,#,Fogne4,acum50,-1,-1;acum100 "acum100" true true false 19 Double 0 0 ,Min,#,Fogne4,acum100,-1,-1', match_option="INTERSECT", search_radius="", distance_field_name="")
